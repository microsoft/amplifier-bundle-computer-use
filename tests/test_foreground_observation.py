...
from amplifier_module_tool_computer_use.foreground import (
    ForegroundObserver,
...
class Backend:
    ...
    def __init__(self):
        ...
        self.window = WindowInfo(
            "42", "Test document", rect=(10, 20, 14, 23), app_name="Fixture app"
        )

    def probe(self):
        return ProbeResult(True)

    def screen_capture_permission(self):
...
@pytest.mark.parametrize(
    "permission,status",
    [(True, "ready"), (False, "permission_required"), (None, "permission_unknown")],
)
def test_status_has_no_window_pixels_input_or_permission_prompt(permission, status):
    backend = Backend()
    backend.permission = permission
...
def test_permission_missing_fails_before_observation(permission):
    ...
    with pytest.raises(ForegroundUnavailable):
        ForegroundObserver(backend).capture()
    assert backend.reads == backend.captures == 0
...
def test_changed_window_drops_pixels():
    ...
    with pytest.raises(ForegroundUnavailable, match="changed") as error:
        ForegroundObserver(backend).capture()
...
def test_permission_revoked_during_capture_drops_pixels():
    ...
    backend.capture = capture
    with pytest.raises(ForegroundUnavailable, match="permission changed"):
        ForegroundObserver(backend).capture()


def test_unknown_bounds_do_not_expand_to_whole_desktop():
    backend = Backend()
    backend.window = WindowInfo("42", "Unknown geometry")
    with pytest.raises(ForegroundUnavailable, match="bounds"):
        ForegroundObserver(backend).capture()
...
def test_native_permission_method_is_read_only(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: seen.append("preflight") or False,
    )
